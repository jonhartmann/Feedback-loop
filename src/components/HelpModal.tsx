    ...
    <div className="help-modal" onClick={onClose}>
      <div className="help-modal__panel" onClick={e => e.stopPropagation()}>
        <div className="help-modal__header">
          <div>
            <div className="help-modal__title">Feedback Loop — Help</div>
            <div className="help-modal__subtitle">
              Build evaluatable node graphs with formulas, units, and what-if simulation
            </div>
          </div>
          <button className="help-modal__close" onClick={onClose} title="Close (Esc)">
            ×
          </button>
        </div>

        <div className="help-modal__body">
          <div className="help-modal__nav">
            {SECTIONS.map(s => (
              <button
                key={s}
                onClick={() => setActive(s)}
...
function Heading({ children }: { children: React.ReactNode }) {
  return <h2 className="help-modal__heading">{children}</h2>
}

function P({ children }: { children: React.ReactNode }) {
  return <p className="help-modal__p">{children}</p>
}
  ...
  return (
    <table className="help-modal__table">
      <thead>
        <tr>
          {headers.map(h => (
            <th key={h} className="help-modal__table-th">{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {rows.map((row, i) => (
          <tr key={i}>
            {row.map((cell, j) => (
              <td key={j} className="help-modal__table-td">{cell}</td>
            ))}
          </tr>
        ))}
      </tbody>
...
function Code({ children }: { children: string }) {
  return <code className="help-modal__code">{children}</code>
}

function CodeBlock({ children }: { children: string }) {
  return <pre className="help-modal__code-block">{children}</pre>
}
...
function NodeTypeCard({ badge, color, label, children }: { badge: string; color: string; label: string; children: React.ReactNode }) {
  return (
    <div className="help-modal__node-type-card">
      <div className="help-modal__node-type-card-label-row">
        <Badge color={color}>{badge}</Badge>
        <span className="help-modal__node-type-card-name">+ {label}</span>
      </div>
      <P>{children}</P>
    </div>
  )
...
function SectionOverview({ onStartTour }: { onStartTour: () => void }) {
  return (
    <>
      ...
      <div className="help-modal__tour-banner">
        <div>
          <div className="help-modal__tour-banner-title">New here? Take the interactive tour</div>
          <div className="help-modal__tour-banner-desc">A guided walkthrough of all the key features — takes about 2 minutes.</div>
        </div>
        <button className="help-modal__tour-btn" onClick={onStartTour}>
          Start tour →
        </button>
      </div>
...
function SectionNodeTypes() {
  return (
    <>
      ...
      <NodeTypeCard badge="Constant" color="#733917" label="Constant">
      ...
      <NodeTypeCard badge="Metric" color="#BF5F0B" label="Metric">
